...
export default function Card({ card, count = null, isLocked = false, onAdd, onRemove, onClick, canAdd = true }) {
  ...
  return (
    ...
    >
      ...
      <div className="text-[9px] md:text-[11px] text-slate-100 text-center bg-slate-950/95 backdrop-blur-md p-1 md:p-1.5 rounded relative flex-1 min-h-[40px] flex flex-col items-center justify-center overflow-y-auto hide-scrollbar z-10 font-medium border border-white/10 w-full shadow-[inset_0_0_10px_rgba(0,0,0,0.8)] mb-1">
        {/* ✨ items-baseline을 사용하여 텍스트 하단 라인과 아이콘을 정렬합니다. */}
        <div className="w-full flex flex-wrap items-baseline justify-center px-0.5 gap-1 break-keep leading-snug">
