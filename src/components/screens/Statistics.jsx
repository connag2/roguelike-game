...
export default function Statistics({
  ...
  const totalCards = CARD_LIBRARY.length + LOOT_CARDS.length;
  ...
  const ACHIEVEMENTS = [
    ...
    { id: 'kill_100', title: '학살자', desc: '적을 누적 100명 처치하세요.', condition: safeGameStats.totalKills >= 100, icon: '🩸', bg: 'from-red-800 to-rose-700', border: 'border-red-500' },
    { id: 'kill_1000', title: '사신', desc: '적을 누적 1,000명 처치하세요.', condition: safeGameStats.totalKills >= 1000, icon: '☠️', bg: 'from-neutral-900 to-red-950', border: 'border-red-500' },
    ...
    { id: 'rich_3', title: '만수르', desc: '누적 100,000 크레딧을 모으세요.', condition: safeGameStats.totalCreditsEarned >= 100000, icon: '💎', bg: 'from-cyan-900 to-blue-800', border: 'border-cyan-500' },
    ...
    { id: 'relic_20', title: '골동품 애호가', desc: '유물을 20개 이상 해금하세요.', condition: unlockedRelics.length >= 20, icon: '🎭', bg: 'from-orange-900 to-orange-600', border: 'border-orange-500' },
    { id: 'relic_all', title: '인디아나 존스', desc: '모든 유물을 해금하세요.', condition: unlockedRelics.length >= RELIC_LIBRARY.length, icon: '🤠', bg: 'from-amber-900 to-orange-700', border: 'border-amber-500' },
    { id: 'card_50', title: '카드 수집가', desc: '도감의 카드를 50% 이상 해금하세요.', condition: unlockedCards.length >= (totalCards * 0.5), icon: '🎴', bg: 'from-blue-900 to-blue-700', border: 'border-blue-500' },
    { id: 'card_all', title: '카드 마스터', desc: '모든 카드를 해금하세요.', condition: unlockedCards.length >= totalCards, icon: '🃏', bg: 'from-indigo-900 to-blue-700', border: 'border-indigo-500' },
    ...
    { id: 'run_50', title: '불굴의 의지', desc: '게임을 누적 50회 플레이하세요.', condition: safeGameStats.totalRuns >= 50, icon: '🔥', bg: 'from-red-950 to-orange-800', border: 'border-orange-500' },
    { id: 'run_100', title: '망령', desc: '게임을 누적 100회 플레이하세요.', condition: safeGameStats.totalRuns >= 100, icon: '👻', bg: 'from-zinc-800 to-zinc-600', border: 'border-zinc-400' },
  ];
      ...
      <div className="w-full max-w-6xl bg-gradient-to-r from-amber-900/40 to-orange-900/40 p-6 md:p-8 rounded-3xl border border-amber-700 shadow-2xl mb-10 shrink-0">
        ...
        <div className="flex flex-col gap-4">
          {MILESTONES.map((ms, index) => {
            ...
            return (
              <div key={index} className={`flex flex-col md:flex-row justify-between items-center p-4 rounded-xl border ${isUnlocked && !isClaimed ? 'bg-amber-900/60 border-amber-500 shadow-[0_0_15px_rgba(245,158,11,0.2)]' : 'bg-slate-900/40 border-slate-700'}`}>
                ...
                <div className="w-full md:w-auto flex justify-end">
                  {isClaimed ? (
                    ...
                    <button 
                      ...
                      className="w-full md:w-auto bg-gradient-to-r from-amber-600 to-orange-500 hover:from-amber-500 hover:to-orange-400 text-white font-bold py-2 px-6 rounded-xl shadow-lg transition-all"
                    >
                      보상 수령
                    </button>
                  ) : (
            ...
            return (
              <div 
                ...
                className={`relative p-4 rounded-2xl border-2 transition-all duration-500 overflow-hidden group 
                  ${isUnlocked ? `bg-gradient-to-br ${ach.bg} ${ach.border} shadow-[0_0_15px_rgba(0,0,0,0.5)] hover:-translate-y-1 hover:shadow-xl` : 'bg-slate-900 border-slate-800 opacity-60 grayscale'}`}
              >
