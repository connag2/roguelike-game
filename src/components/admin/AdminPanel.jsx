...
`.trim();
  ...
  const handleInfiniteMana = () => { if(checkCombat()) setCombatState(prev => ({ ...prev, player: { ...prev.player, mana: 99, maxMana: 99 } })); setToastMsg("최대/현재 마나 99 세팅!"); };
  ...
  const handleEnemyDebuffBomb = () => {
    ...
    setCombatState(prev => ({
      ...
      enemies: prev.enemies.map(e => ({
        ...e, debuffs: { ...e.debuffs, vulnerable: (e.debuffs?.vulnerable||0)+99, weak: (e.debuffs?.weak||0)+99, poison: (e.debuffs?.poison||0)+99 }
      }))
    }));
  ...
  const handleMaxHpChange = (amount) => {
    if(checkCombat()) {
      ...
      setToastMsg(`최대 체력 ${amount > 0 ? '+'+amount : amount} 적용`);
    }
  };
  ...
  const handleRemoveSpecificCard = () => {
    ...
    setDeckCounts(prev => {
      ...
      if (newCounts[selectedCardId] > 0) {
        ...
        setToastMsg(`[${CARD_LIBRARY.find(c => c.id === selectedCardId)?.name}] 덱에서 1장 제거`);
      } else {
  ...
  const handleAddSpecificRelic = () => {
    ...
    setPlayerRelics(prev => {
      if(prev?.find(r => r.id === selectedRelicId)) { setToastMsg("이미 보유한 유물입니다."); return prev; }
      return [...(prev || []), relicDef];
    });
    setToastMsg(`[${relicDef.name}] 장착!`); saveGame();
  };
  ...
  const handleAddRandomRelic = () => {
    ...
    if(unowned.length > 0) {
      ...
      setToastMsg(`[${randomRelic.name}] 무작위 획득!`); saveGame();
  ...
  const handleDuplicateHand = () => {
    if(!checkCombat()) return;
    setCombatState(prev => ({
      ...prev, hand: [...prev.hand, ...prev.hand.map(c => ({...c, uid: Math.random().toString()}))]
    }));
  ...
  // 3. 시스템/자원/이동 기능
  const handleAddGold = (amount) => { setCredits(prev => Math.max(0, prev + amount)); setToastMsg(`${amount > 0 ? '+'+amount : amount} 크레딧!`); saveGame(); };
              ...
              {activeTab === 'combat' && (
                <div className="flex flex-col gap-3">
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <button onClick={handleGodMode} className="bg-blue-700 hover:bg-blue-600 p-2 rounded text-[11px] font-bold">🛡️ 무적 (방어도 9999)</button>
                    <button onClick={handleKillPlayer} className="bg-red-900 hover:bg-red-800 p-2 rounded text-[11px] border border-red-500">🩸 체력 1로</button>
                  ...
                  <div className="bg-slate-800/80 p-3 rounded-lg border border-slate-700">
                    <p className="text-xs text-sky-300 font-bold mb-2">패/드로우 조작</p>
                  ...
                  <div className="bg-slate-800/80 p-3 rounded-lg border border-slate-700">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <button onClick={handleForceWin} className="col-span-2 bg-yellow-600 hover:bg-yellow-500 p-2 rounded text-xs font-bold text-black mt-1">🏆 즉시 전투 승리</button>
                    </div>
                  </div>
                </div>
              )}
              ...
              {activeTab === 'deck' && (
                <div className="flex flex-col gap-4">
                  <div className="bg-slate-800/80 p-3 rounded-lg border border-slate-700 flex flex-col gap-2">
                    ...
                    <div className="flex gap-2">
                      <button onClick={handleAddSpecificCard} className="flex-1 bg-amber-700 hover:bg-amber-600 py-1.5 rounded text-[11px] font-bold">덱에 추가</button>
                      <button onClick={handleRemoveSpecificCard} className="flex-1 bg-slate-700 hover:bg-slate-600 py-1.5 rounded text-[11px]">덱에서 1장 제거</button>
                    </div>
