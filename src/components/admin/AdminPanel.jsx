...
const AdminPanel = ({ 
  ...
  const { playSFX } = useAudio(); // 사운드 훅 사용
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      if (e.ctrlKey && e.shiftKey && e.key.toLowerCase() === 'a') {
        setIsVisible(prev => {
          if (!prev) setPwdInput(''); // 패널을 열 때 비밀번호 입력칸 초기화
          return !prev;
        });
      }
    };
  ...
  // ✨ [추가] 인증 로직
  const handleAuth = () => {
    if (pwdInput === '20090324') {
      setIsAuthenticated(true);
      setPwdInput('');
      if(playSFX) playSFX('buy_item'); // 성공 시 상점 짤랑 소리
      setToastMsg('🔓 관리자 패널 잠금 해제!');
    } else {
      if(playSFX) playSFX('damage_take'); // 실패 시 타격 소리
  ...
  const checkCombat = () => {
    if (!combatState) { setToastMsg("전투 중에만 사용 가능합니다."); return false; }
    return true;
  };

  // 기존 전투/시스템 조작 함수들 (유지)
  const handleMaxHealth = () => { if(checkCombat()) { setCombatState(prev => ({ ...prev, player: { ...prev.player, hp: prev.player.maxHp } })); setToastMsg("✅ 체력 풀회복!"); } };
  const handleGodMode = () => { if(checkCombat()) { setCombatState(prev => ({ ...prev, player: { ...prev.player, block: 9999 } })); setToastMsg("🛡️ 방어도 9999!"); } };
  const handleInfiniteMana = () => { if(checkCombat()) { setCombatState(prev => ({ ...prev, player: { ...prev.player, mana: 99, maxMana: 99 } })); setToastMsg("🔵 마나 99!"); } };
  const handleKillAllEnemies = () => { if(checkCombat()) { setCombatState(prev => ({ ...prev, enemies: [] })); setToastMsg("☠️ 모든 적 제거!"); } };
  const handleForceWin = () => { if(checkCombat()) { setGameState('REWARDS'); setToastMsg("🏆 전투 즉시 승리!"); } };
  const handleAddBuffs = () => { if(checkCombat()) { setCombatState(prev => ({ ...prev, player: { ...prev.player, buffs: { ...prev.player.buffs, strength: (prev.player.buffs.strength||0)+50, dexterity: (prev.player.buffs.dexterity||0)+50 } } })); setToastMsg("💪 힘/민첩 +50!"); } };
  const handleClearDebuffs = () => { if(checkCombat()) { setCombatState(prev => ({ ...prev, player: { ...prev.player, debuffs: { weak:0, vulnerable:0, poison:0, frail:0, mark:0, silence:0, bind:0 } } })); setToastMsg("❌ 모든 디버프 해제!"); } };
  const handleDrawCards = (count) => { if(!checkCombat()) return; setCombatState(prev => { let drawPile = [...prev.drawPile]; let hand = [...prev.hand]; for(let i=0; i<count; i++) { if(drawPile.length > 0) hand.push(drawPile.pop()); } return { ...prev, drawPile, hand }; }); setToastMsg(`🃏 ${count}장 드로우!`); };
  const handleEnemyDebuffBomb = () => { if(!checkCombat()) return; setCombatState(prev => ({ ...prev, enemies: prev.enemies.map(e => ({ ...e, debuffs: { ...e.debuffs, vulnerable: (e.debuffs?.vulnerable||0)+99, weak: (e.debuffs?.weak||0)+99, poison: (e.debuffs?.poison||0)+99 } })) })); setToastMsg("💀 적 디버프 99 부여!"); };
  ...
  const handleAddRelic = () => {
    ...
    if (!relicDef) return;
    if(playerRelics?.find(r => r.id === selectedRelicId)) { setToastMsg("⚠️ 이미 보유한 유물입니다."); return; }
    setPlayerRelics(prev => [...(prev || []), relicDef]);
    setToastMsg(`💎 [${relicDef.name}] 추가!`);
  };

  const handleUnlockAllCards = () => {
    const allCardIds = [...CARD_LIBRARY.map(c => c.id), ...BOSS_LOOT_CARDS.map(c => c.id)];
  ...
  const handleUnlockAll = () => { handleUnlockAllCards(); handleUnlockAllRelics(); };

  const handleAddGold = (amount) => { 
    const newAmount = Math.max(0, credits + amount);
    setCredits(newAmount); setToastMsg(`💰 크레딧 ${amount > 0 ? '+'+amount : amount}!`); saveGame({ credits: newAmount });
  };
  ...
  return (
    <div 
      className="fixed z-[99999] bg-slate-900/95 backdrop-blur-md border-2 border-fuchsia-600 rounded-lg shadow-[0_0_30px_rgba(192,38,211,0.3)] overflow-hidden flex flex-col text-white font-sans transition-all duration-200"
      style={{ left: position.x, top: position.y, width: '540px', maxHeight: '90vh', opacity: isDragging ? 0.7 : 1 }}
    >
      {/* 헤더 */}
      <div className="bg-fuchsia-900/60 p-3 flex justify-between items-center cursor-move border-b border-fuchsia-700" onMouseDown={handleDragStart}>
        <div className="flex items-center gap-2 pointer-events-none">
          {isAuthenticated ? <Unlock className="w-5 h-5 text-fuchsia-300"/> : <Lock className="w-5 h-5 text-slate-400"/>}
          <h2 className="font-bold text-fuchsia-200 text-sm tracking-wide">DEV PANEL</h2>
        </div>
        <div className="flex gap-2 items-center">
          {/* ✨ 수동 잠금 버튼 */}
          {isAuthenticated && (
            <button 
              onClick={() => { setIsAuthenticated(false); if(playSFX) playSFX('click'); }} 
              className="text-fuchsia-200 hover:text-white px-2 py-1 text-xs bg-fuchsia-800/80 hover:bg-fuchsia-600 rounded flex items-center gap-1 transition"
            >
              <Lock className="w-3 h-3"/> 잠금
            </button>
          )}
          <button onClick={() => setIsMinimized(!isMinimized)} className="text-slate-300 hover:text-white px-2 py-1 text-xs bg-slate-800 rounded transition">
            {isMinimized ? '▼' : '▲'}
          </button>
          <button onClick={() => setIsVisible(false)} className="text-red-400 hover:text-red-300 font-bold px-2 transition">✕</button>
        </div>
      </div>
      ...
      {!isMinimized && (
        <div className="p-4 overflow-y-auto custom-scrollbar" style={{ maxHeight: 'calc(90vh - 50px)' }}>
          ...
          {/* ✨ 잠금 화면 (인증 전) */}
          {!isAuthenticated ? (
            <div className="flex flex-col items-center justify-center py-10 px-6 gap-5 animate-in fade-in slide-in-from-bottom-4 duration-300">
              <div className="w-16 h-16 bg-fuchsia-900/30 rounded-full flex items-center justify-center mb-2 shadow-[0_0_15px_rgba(192,38,211,0.5)]">
                <KeyRound className="w-8 h-8 text-fuchsia-400 animate-pulse" />
              </div>
              <p className="text-sm text-slate-300 font-medium text-center">
                패널을 열기 위해<br/>접근 코드를 입력해 주세요.
              </p>
              <div className="flex gap-2 w-full max-w-xs">
                <input 
                  ...
                  placeholder="********" 
                  className="flex-1 bg-slate-950 border border-fuchsia-800/50 rounded-lg px-4 py-2 text-center text-white outline-none focus:border-fuchsia-400 focus:shadow-[0_0_10px_rgba(192,38,211,0.4)] tracking-[0.3em] transition-all"
                />
              </div>
              <button 
                onClick={handleAuth} 
                className="w-full max-w-xs py-2.5 bg-fuchsia-700 hover:bg-fuchsia-600 rounded-lg font-bold text-sm shadow-lg transition-colors"
              >
                접근 승인 요청
              </button>
            </div>
