...
// 외부로 분리한 데이터와 로직 임포트
import { CARD_LIBRARY, BASE_CARDS, GAME_RULES, ENEMIES, NORMAL_BOSSES, SPECIAL_BOSSES } from './constants/gameData';
import { shuffle, decayStack, getCardDef, generateEnemies, generateEnemyIntent, validateDeckStatus, getTotalCards, startNextStage, renderFiltersUI, renderTooltipIcon } from './utils/gameLogic';
...
export default function App() {
  ...
  // --- 누락된 더미 핸들러 (이게 없으면 앱이 하얗게 멈춤) ---
  const handleImport = (text) => showToast('Import Not Implemented in this snippet');
  const handleExport = () => showToast('Export Not Implemented in this snippet');
  const handleExitGame = () => setGameState('MENU');
  
  // 렌더 메뉴 더미 구현 (사용자가 일부 잘라냈기 때문에 연결만 해줌)
  const renderMenu = () => (
    <div className="flex flex-col items-center justify-center min-h-screen text-white">
      <h1 className="text-6xl font-black mb-12 tracking-tighter text-indigo-500">ROGUELIKE TACTICS</h1>
      <div className="flex flex-col gap-4 w-64">
        <button onClick={() => setGameState('BATTLE')} className="py-4 bg-indigo-600 rounded-xl font-black">START MISSION</button>
        <button onClick={() => setGameState('DECK_BUILDING')} className="py-4 bg-slate-800 rounded-xl font-black">DECK BUILDER</button>
        <button onClick={() => setGameState('ENCYCLOPEDIA')} className="py-4 bg-slate-800 rounded-xl font-black">ARCHIVE</button>
        <button onClick={() => setGameState('MONSTER_DEX')} className="py-4 bg-slate-800 rounded-xl font-black">MONSTER DEX</button>
        <button onClick={() => setGameState('SETTINGS')} className="py-4 bg-slate-800 rounded-xl font-black">SETTINGS</button>
      </div>
    </div>
  );
  ...
  const renderShop = () => (
    <div className="flex flex-col items-center justify-center min-h-screen text-white">
      <h2 className="text-4xl font-black mb-8">SHOP & UPGRADES</h2>
      <button onClick={() => setGameState('MENU')} className="px-6 py-2 bg-slate-700 rounded-xl">BACK</button>
    </div>
  );
  ...
  const playCard = (cardIndex) => {
    ...
    setCombatState(prev => {
      // React 불변성 유지를 위해 깊은 복사
      let p = { 
        ...prev.player,
        buffs: { ...(prev.player.buffs || {}) },
        debuffs: { ...(prev.player.debuffs || {}) }
      };
      let newEnemies = prev.enemies.map(e => ({
        ...e,
        debuffs: { ...(e.debuffs || {}) },
        passives: e.passives ? [...e.passives] : []
      }));
      let newHand = [...prev.hand], newDiscard = [...prev.discardPile], newDraw = [...prev.drawPile];
      
      p.mana -= card.cost;
      
      const isWin = !card.gamble || Math.random() < (card.gambleWinChance || 0);
      ...
      const deal = (amt) => {
        ...
        if (targetBlock >= dmg) {
          target.block = targetBlock - dmg;
        } else {
          target.hp -= (dmg - targetBlock); 
          target.block = 0; 
        }

        if (target.hp <= 0) {
          // TypeError 방지를 위한 옵셔널 체이닝 추가 (?.)
          const revIdx = target.passives?.findIndex(ps => ps.id === 'revive');
          if (revIdx !== undefined && revIdx > -1) { 
            target.hp = Math.floor(target.maxHp / 2); 
            target.passives.splice(revIdx, 1); 
            showToast('부활!'); 
          } else {
            newEnemies.shift();
          }
      ...
      if (isWin) {
        if (card.winDamage) deal(newEnemies[0]?.isBoss && card.winDamageBoss ? card.winDamageBoss : card.winDamage);
        if (card.winManaGain) p.mana += card.winManaGain;
        if (card.winHeal) p.hp = Math.min(p.maxHp, p.hp + card.winHeal);
        if (card.percentBlockMaxHp) p.block = (p.block || 0) + Math.floor(p.maxHp * (card.percentBlockMaxHp / 100)) + (p.buffs.dexterity || 0);
        if (card.doubleBlock) p.block = (p.block || 0) * 2;
        ...
        if (card.block && !card.doubleBlock && !card.percentBlockMaxHp) p.block = (p.block || 0) + card.block + (p.buffs.dexterity || 0);
      ...
      // 드로우 로직
      let maxHand = GAME_RULES?.MAX_HAND_SIZE || 10;
      for(let i=0; i<(card.draw || 0); i++) {
        if(newHand.length >= maxHand - 1) break;
        if(newDraw.length === 0) { 
          if(newDiscard.length === 0) break; 
          newDraw = shuffle ? shuffle(newDiscard) : newDiscard.sort(() => Math.random() - 0.5); 
          newDiscard = []; 
        }
    ...
    return (
      <div key={card.uid || card.id} onClick={customClick} className={`border-2 p-2 rounded-xl flex flex-col justify-between relative transition-all ${customClick ? 'cursor-pointer hover:-translate-y-2' : ''} ${isLocked ? 'opacity-40 grayscale border-slate-700 bg-slate-900' : `${border} ${shadow} ${bg}`} w-full aspect-[3/4.2] max-w-[160px] mx-auto overflow-hidden shadow-2xl`}>
        ...
        <div className="text-[10px] text-slate-200 text-center leading-tight bg-black/60 p-2 rounded flex-1 flex items-center justify-center font-medium z-10 mt-1 border border-slate-800/50">
          <div>{card.desc} {typeof renderTooltipIcon === 'function' ? renderTooltipIcon(card.desc) : null}</div>
        </div>
  ...
  // --- 카드 필터링 엔진 ---
  const getFilteredCardsSafe = (tFilter, eFilter, rFilter, oFilter, query) => {
    if (!CARD_LIBRARY) return [];
    return CARD_LIBRARY.filter(c => {
      ...
      if (query) {
        ...
        const def = typeof getCardDef === 'function' ? getCardDef(c.id, shopUpgrades) : c;
  ...
  const renderEncyclopedia = () => {
    const cards = getFilteredCardsSafe(filterType, filterEffect, filterRarity, filterOwnership, searchQuery);
    return (
      <div className="flex flex-col min-h-[100dvh] bg-slate-900 text-white p-4 pt-16">
        ...
        <div className="max-w-6xl mx-auto w-full mb-8">
          {typeof renderFiltersUI === 'function' && renderFiltersUI(filterType, setFilterType, filterEffect, setFilterEffect, filterRarity, setFilterRarity, filterOwnership, setFilterOwnership, searchQuery, setSearchQuery)}
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6 max-w-6xl mx-auto w-full overflow-y-auto pb-20 pr-2 custom-scroll">
          {cards.map((c, i) => renderCard(typeof getCardDef === 'function' ? getCardDef(c.id, shopUpgrades) : c, null, !unlockedCards.includes(c.id)))}
        </div>
      </div>
    );
  };
  ...
  const renderMonsterDex = () => {
    const all = [...(ENEMIES || []), ...(NORMAL_BOSSES || []), ...(SPECIAL_BOSSES ? Object.values(SPECIAL_BOSSES) : [])];
    return (
      <div className="flex flex-col min-h-[100dvh] bg-slate-900 text-white p-4 pt-16">
        <div className="flex justify-between items-center mb-8 max-w-6xl mx-auto w-full px-4">
      ...
      <div className="flex flex-col h-[100dvh] bg-slate-950 text-white p-2 md:p-4 overflow-hidden relative">
        ...
        <div className="flex-1 flex flex-row justify-between items-center max-w-7xl mx-auto w-full px-6 relative z-10">
          ...
          <div className="flex gap-12 items-end">
            {enemies.map((e, idx) => {
              const intent = e.intentCard || { name: 'Unknown', desc: '...', type: '' };
              const isTarget = idx === 0;
              return (
                <div key={e.uid || idx} className={`flex flex-col items-center transition-all duration-500 ${isTarget ? 'scale-100' : 'scale-75 opacity-40 translate-x-10 blur-[1px]'}`}>
      ...
      <div className="flex flex-col h-[100dvh] bg-slate-900 text-white p-4 pt-16">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 max-w-6xl mx-auto w-full gap-4 px-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
             <span className={`text-lg font-black mr-4 ${typeof getTotalCards === 'function' && getTotalCards(tempDeckCounts) === 20 ? 'text-green-400' : 'text-yellow-500'}`}>
               TOTAL: {typeof getTotalCards === 'function' ? getTotalCards(tempDeckCounts) : 0} / 20
             </span>
             ...
             <button onClick={() => { setDeckCounts(tempDeckCounts); saveGame({ deckCounts: tempDeckCounts }); showToast('DECK SAVED!'); }} className="px-6 py-2 bg-emerald-600 hover:bg-emerald-500 rounded-xl font-bold shadow-lg border-b-4 border-emerald-800">SAVE DECK</button>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-5 max-w-6xl mx-auto w-full overflow-y-auto pb-20 pr-2 custom-scroll">
          {cards.map((c, i) => {
            ...
            return renderCard(typeof getCardDef === 'function' ? getCardDef(c.id, shopUpgrades) : c, count, isLocked, handleAddCard, handleRemoveCard);
          })}
        </div>
      </div>
    );
  ...
  const renderRewards = () => (
    <div className="flex flex-col items-center justify-center min-h-[100dvh] bg-slate-950 text-white p-4">
      ...
      <div className="flex flex-col md:flex-row gap-8 max-w-5xl w-full justify-center">
        <div onClick={() => {
          const pool = CARD_LIBRARY ? CARD_LIBRARY.filter(c => c.rarity !== 'special') : [];
          if (pool.length === 0) return;
          let res = []; 
          for(let i=0; i<3; i++) {
            const picked = pool[Math.floor(Math.random()*pool.length)];
            res.push(typeof getCardDef === 'function' ? getCardDef(picked.id, shopUpgrades) : picked);
          }
        ...
        <div onClick={() => {
          ...
          if (typeof startNextStage === 'function') startNextStage(p, combatState.baseDeck);
          else showToast('startNextStage not found');
          ...
          <div className="bg-slate-900 p-10 rounded-[3rem] border-2 border-indigo-500/50 w-full max-w-md text-center shadow-[0_0_50px_rgba(79,70,229,0.2)] animate-draw flex flex-col items-center">
            ...
            <div className="flex gap-4 w-full">
              ...
              <button onClick={() => {
                ...
                if (typeof startNextStage === 'function') startNextStage(combatState.player, newDeck);
  ...
  const renderGameOver = () => (
    <div className="flex flex-col items-center justify-center min-h-[100dvh] bg-black text-white p-4">
      ...
      <div className="text-xl font-bold text-slate-500 tracking-widest uppercase mb-16 flex items-center gap-4">
         Stage Reached <span className="text-white bg-red-900/50 px-4 py-1 rounded-lg border border-red-500/30">{combatState?.stage || 1}</span>
      </div>
  ...
  const renderGameClear = () => (
    <div className="flex flex-col items-center justify-center min-h-[100dvh] bg-slate-950 text-white p-4 relative">
      ...
      <h1 className="text-7xl font-black text-yellow-600 mb-6 text-center">CONQUEROR</h1>
      <button onClick={() => setGameState('MENU')} className="px-16 py-6 bg-yellow-600 text-black rounded-3xl font-black text-2xl">RETIRE IN GLORY</button>
    </div>
  );
